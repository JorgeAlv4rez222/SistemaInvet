  ...
  return (
    <div className="notas-page">
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
        ...
        />
        {(['todos', 'libre', 'completado', 'parcial', 'sin_stock'] as const).map((f) => (
          <button
            key={f}
            ...
            style={{
              ...
              background: filtroEstado === f
                ...
                : 'var(--primary)'
                : 'var(--surface)',
              color: filtroEstado === f ? (f === 'todos' || f === 'libre' ? 'white' : '#000') : 'var(--text-secondary)',
            }}
          >
            {f === 'todos' ? 'Todos' : f === 'libre' ? 'Pendiente' : f === 'completado' ? 'Completas' : f === 'parcial' ? 'Parcial' : 'Sin stock'}
          </button>
        ))}
      </div>
